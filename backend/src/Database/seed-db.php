...
// Carrega autoloader e variáveis de ambiente para garantir o caminho correto do banco
...
// =========================
$empresas = [
    ['WS landpager Ltda', 'Gerenciando o futuro', 'Sistema completo de gestão', '12.345.678/0001-91', 'Tecnologia', 'Brasil', 'Organizar processos', 'Sistemas Web', 'WS Platform'],
...
foreach ($empresas as $e) { $stmt->execute($e); }
logMsg("Empresas ok");
...
foreach ($gestoes as $g) { $stmt->execute($g); }
...
foreach ($calendario as $c) { $stmt->execute($c); }
...
foreach ($emprestimos as $e) { $stmt->execute($e); }
...
foreach ($lancamentos as $l) { $stmt->execute($l); }
...
foreach ($manutencao as $m) { $stmt->execute($m); }
...
foreach ($servidores as $s) { $stmt->execute($s); }
...
foreach ($disps as $d) { $stmt->execute($d); }
...
foreach ($redes as $r) { $stmt->execute($r); }
...
foreach ($contratos as $c) { $stmt->execute($c); }
...
foreach ($cursos as $c) { $stmt->execute($c); }
...
foreach ($plataformas as $p) { $stmt->execute($p); }
...
foreach ($invest as $i) { $stmt->execute($i); }
